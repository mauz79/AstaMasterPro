...
// Serie A – Ricerca, schede Corrente/Precedente/Storico, ranking ruolo, opzioni
(function(){
  'use strict';

  // ====== Config ======
  var DATA_DIRS = ['data/']; // ricerca sempre nella cartella /data alla root
  var IMG_BASE  = 'img/';
  ...
  // ====== Foto giocatore (fallback multiplo) ======
  ...
  function setupTheme(){
    ...
    var sw=$('#switchTheme'); if(sw) sw.addEventListener('change', function(e){
      var mode=e.target.checked?'dark':'light'; applyTheme(mode); localStorage.setItem(THEME_KEY, mode);
    });
  }
  ...
  function loadOptions(){
    ...
    set('opt_showDelta',o.showDelta,true); set('opt_showDetails',o.showDetails,true);
    set('opt_histShowBM',o.histShowBM,true); set('opt_histShowPerPres',o.histShowPerPres,true);
    set('opt_useIndicators',o.useIndicators,true); set('opt_showRolePos',o.showRolePos,true);
    return o;
  }
  ...
  // ====== Caricamento stagioni (auto-scan) ======
  ...
  function fileCandidatesFor(key){
    ...
    var next = start+1;
    // supporto: "YYYY_YYYY+1.json", "YYYY.json", "YYYY-YYYY+1.json"
  ...
  function tryFetch(path){
    return fetch(path,{cache:'no-store'}).then(function(res){ if(!res.ok) return null; return res.json(); })
  ...
  async function loadSeasonAuto(key){
    for(var d=0; d<DATA_DIRS.length; d++){
      var dir=DATA_DIRS[d]; var files=fileCandidatesFor(key);
  ...
  async function discoverSeasons(){
    var keys=candidateSeasonKeys(); var out=[];
    ...
    if(!out.length){
      console.error('Nessuna stagione trovata. Assicurati che i JSON siano in /data e si chiamino YYYY_YYYY+1.json o YYYY.json o YYYY-YYYY+1.json');
    } else {
  ...
  function renderResults(items,q){
    ...
    items.slice(0,120).forEach(function(item){
      ...
      var badges=[]; if(item.changedRole) badges.push('<span class="badge warn" title="Cambio ruolo vs Stag. prec.">Cambio Ruolo</span>');
  ...
  function getRoleDistribution(seasonObj, role, metric, minPres){
    ...
    if(!cacheSeason[r][key]){
      ...
      var N=vals.length; var mean=N?vals.reduce(function(a,b){ return a+b; },0)/N:0;
  ...
  function computeRankForSeason(seasonObj, metric, value, role, minPres){
    try{
      ...
      var mid=less + 0.5*equal; var pct=mid/N; var z=std>0 ? (v-mean)/std : 0; var rank=Math.max(1, N-(less+equal)+1);
  ...
  // NEW: riga con label HTML ma valore come intero (per i conteggi richiesti)
  ...
  function renderCurrent(rec){
    ...
    // Interi per i conteggi (portieri e non)
    if(isP){
      parts.push(rowHTMLInt(labelWithMark('GS',  'gol_subiti',        '🔴🧠'), rec.gs));
      parts.push(rowHTMLInt(labelWithMark('GSR', 'gol_subiti_rigore', '🅁🔴🧠'), rec.gsr));
      parts.push(rowHTMLInt(labelWithMark('Rigori Parati', 'rigori_parati','🅁🟢🧠'), rec.rp));
    } else {
      parts.push(rowHTMLInt(labelWithMark('Gol',           'gol',        '⚽'),    rec.gf));
      parts.push(rowHTMLInt(labelWithMark('Gol su Rigore', 'gol_rigore', '🅁⚽'),   rec.gfr));
      parts.push(rowHTMLInt(labelWithMark('Rigori Sbagliati','rigori_sbagliati','🅁🔴⚽'), rec.rs));
    }
    ...
    if(OPTS.showRolePos){
      ...
      if(rkFM||rkMV){
        ...
        if(rkMV){ lines.push('<div class="row"><span class="key">MV '+escapeHtml(y)+' – Percentile</span><span class="val">'+fmt(rkMV.pct*100,1).replace('.', ',')+'%</span></div>'); lines.push(row('MV – Z‑score', rkMV.z)); }
  ...
  // ====== Badge header dalla stagione precedente ======
  ...
  function selectPlayer(cod){
    ...
    var hdr=[]; if(idx && idx.changedRole) addBadge(hdr, 'Cambio Ruolo', 'warn'); if(idx && idx.changedTeam) addBadge(hdr, 'Cambio Squadra', 'alert');
    if(PREV && prevRec){ buildRankingBadges(prevRec, idx).forEach(function(x){ hdr.push(x); }); }
    $('#playerBadges').innerHTML = hdr.join(' ');

    renderCurrent(curRec); renderPrev(prevRec, PREV ? PREV.seasonKey : null); renderStorico(cod);
    $('#results').innerHTML=''; hideResults();
  }

  // ====== Ricerca: input & hotkey ======
  function wireSearch(){
  ...
  function init(){
    ...
    discoverSeasons().then(function(list){
      ...
      $('#lblPrev').textContent    = PREV ? ('Stagione ' + seasonStr(PREV.seasonKey)) : 'Stagione precedente';
      buildIndex(); populateHistSeasonOptions();
