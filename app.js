// AstaMasterPro v9c2 • DIAGNOSTICA — app.js
(function(){
  'use strict';

  // ==== Config & helpers ====
  var DATA_BASE = null; // chosen dynamically (data/ or 7data/ or Data/ or DATA/)
  var DATA_DIRS = ['data/', '7data/', 'Data/', 'DATA/'];
  var IMG_BASE = 'img/';
  var SCAN_BACK_YEARS = 8, SCAN_FWD_YEARS = 1;
  ...
  function highlightMatch(t,q){ var lt=String(t==null?'':t), qq=String(q==null?'':q); var i=lt.toLowerCase().indexOf(qq.toLowerCase()); if(i<0) return escapeHtml(lt); var e=i+qq.length; return escapeHtml(lt.slice(0,i))+'<mark>'+escapeHtml(lt.slice(i,e))+'</mark>'+escapeHtml(lt.slice(e)); }
  function seasonStr(key){ return String(key || '').replace('_','/'); }
  ...
  function setPlayerPhoto(cod){
    ...
    img.onerror = function(){ next(); }; next();
  }
  ...
  function applyToggles(){
    ...
    var el;
    el = $('#switchCurrent'); if(el) el.checked = bCur; el = $('#wrapCurrent'); if(el) el.classList.toggle('hidden', !bCur);
  ...
  function setupToggles(){
    var el;
    el=$('#switchCurrent'); if(el) el.addEventListener('change', function(e){ localStorage.setItem(TGL_KEYS.cur, e.target.checked?'on':'off'); applyToggles(); });
    el=$('#switchPrev');    if(el) el.addEventListener('change', function(e){ localStorage.setItem(TGL_KEYS.prev,e.target.checked?'on':'off'); applyToggles(); });
    el=$('#switchStorico'); if(el) el.addEventListener('change', function(e){ localStorage.setItem(TGL_KEYS.hist,e.target.checked?'on':'off'); applyToggles(); });
    applyToggles();
  }
  ...
  function setupOptions(){
    ...
    var r = $('#btnResetOptions'); if(r) r.addEventListener('click', function(){
      ...
      OPTS = loadOptions(); setupTheme(); applyToggles(); populateHistSeasonOptions();
      if(SELECTED_COD) selectPlayer(SELECTED_COD);
  ...
  // ==== Data: autodetect dir + support 2 naming schemes ====
  function candidateSeasonKeys(){ var y=(new Date()).getFullYear(); var keys=[]; for(var yy=y-SCAN_BACK_YEARS; yy<=y+SCAN_FWD_YEARS; yy++) keys.push(yy+'_'+(yy+1)); return keys.reverse(); }
  function fileCandidatesFor(key){ var start=Number(String(key).split('_')[0]); return [key+'.json', start+'.json']; }
  function normRow(obj){
    ...
    return {
      ...
      mvdst: takef('MVDSt'), mvdlt: takef('MVDlt'), fmdst: takef('FMDSt'), fmdlt: takef('FMDlt'),
  ...
  function tryFetch(path){ return fetch(path, {cache:'no-store'}).then(function(res){ if(!res.ok) return null; return res.json(); }).catch(function(){ return null; }); }
  async function loadSeasonAuto(key){
    for(var d=0; d<DATA_DIRS.length; d++){
      ...
      for(var i=0; i<files.length; i++){
        ...
        if(raw){
          if(!DATA_BASE) DATA_BASE = dir;
          var rows = parseRaw(raw); if(!rows) continue;
          var start = Number(String(key).split('_')[0]);
          return { seasonKey:key, seasonStart:start, players:rows };
        }
      }
    }
    return null;
  }
  async function discoverSeasons(){ var keys=candidateSeasonKeys(); var out=[]; for(var i=0;i<keys.length;i++){ var s=await loadSeasonAuto(keys[i]); if(s) out.push(s); } out.sort(function(a,b){ return a.seasonStart - b.seasonStart; }); return out; }

  // ==== Index & search ====
  function buildIndex(){
    INDEX=[]; var prevMap=new Map(); if(PREV) PREV.players.forEach(function(p){ prevMap.set(String(p.cod), p); });
    CUR.players.forEach(function(c){
      ...
      INDEX.push({ cod:String(c.cod), nome:c.nome, ruolo:c.r, squadra:c.sq, prevRuolo: pr?pr.r:null, prevSquadra: pr?pr.sq:null, changedRole:changedRole, changedTeam:changedTeam });
    });
  }
  ...
  function renderResults(items, q){ var ul=$('#results'); if(!ul) return; ul.innerHTML=''; items.slice(0,80).forEach(function(item){ var li=document.createElement('li'); li.className='result-item'; li.setAttribute('role','option'); li.tabIndex=0; var title='<span class="result-title">'+highlightMatch(item.nome, q)+'</span>'; var meta='<span class="result-meta">'+escapeHtml(item.ruolo)+' · '+escapeHtml(item.squadra)+'</span>'; var badges=[]; if(item.changedRole) badges.push('<span class="badge warn" title="Cambio ruolo vs Stag. prec.">Cambio Ruolo</span>'); if(item.changedTeam) badges.push('<span class="badge alert" title="Cambio squadra vs Stag. prec.">Cambio Squadra</span>'); li.innerHTML = title+'<span>'+meta+' '+badges.join(' ')+'</span>'; li.addEventListener('click', function(){ selectPlayer(item.cod); }); li.addEventListener('keydown', function(e){ if(e.key==='Enter') selectPlayer(item.cod); }); ul.appendChild(li); }); }
  function search(q){ var nq=norm(q); var ul=$('#results'); if(!nq){ if(ul) ul.innerHTML=''; return; } var res = INDEX.filter(function(x){ return norm(x.nome).includes(nq); }); renderResults(res, q); }

  // ==== Icons/indicators ====
  function iconOrIndicator(iconKey, indicator){ if(OPTS.useIndicators) return (indicator?indicator+' ':''); if(!iconKey) return ''; return '<img src="'+IMG_BASE+'ico/'+iconKey+'.svg" class="ico stat" alt="" onerror="this.style.display=\'none\'"> '; }
  ...
  function getRoleDistribution(seasonObj, role, metric, minPres){
    if(!seasonObj) return null; var seasonKey = seasonObj.seasonKey; var r=String(role||'').toUpperCase(); if(!r) return null; var cacheSeason = ensureRoleCache(seasonKey); if(!cacheSeason[r]) cacheSeason[r]={}; var key = metric+'__'+String(minPres);
    if(!cacheSeason[r][key]){
      ...
      var N = vals.length; var mean = N ? vals.reduce(function(a,b){ return a+b; },0)/N : 0; var variance = N ? vals.reduce(function(s,x){ var d=x-mean; return s + d*d; },0)/N : 0; var std = Math.sqrt(variance);
  ...
  function computeRankForSeason(seasonObj, metric, value, role, minPres){
    try{
      var v=Number(value); if(!isFinite(v)) return null; var dist=getRoleDistribution(seasonObj, role, metric, minPres); if(!dist || !dist.N) return null; var vals=dist.vals, N=dist.N, mean=dist.mean, std=dist.std; var less=0, equal=0, eps=1e-9; for(var i=0;i<vals.length;i++){ var x=vals[i]; if(x < v - eps) less++; else if(Math.abs(x-v) <= eps) equal++; } var mid = less + 0.5*equal; var pct = mid / N; var z = std>0 ? (v-mean)/std : 0; var rank = Math.max(1, N - (less+equal) + 1); return { pct:pct, z:z, N:N, rank:rank };
    }catch(e){ return null; }
  }
...
function renderCurrent(rec){
  ...
  function renderStorico(cod){
    ...
    var n=filtered.length; var list = filtered.map(function(x){ return seasonStr(x.seasonKey); }).join(', ');
    ...
    var mvW = presTot ? (mvNum/presTot) : null; var fmW = presTot ? (fmNum/presTot) : null;
    var r0 = filtered[0].rec || {}; var isP = (r0.r==='P');
  ...
  // ==== Header badges (previous season) ====
  function buildRankingBadges(prevRec, idx){
    var b=[]; if(!prevRec) return b; var y = seasonStr(PREV.seasonKey);
  ...
  function selectPlayer(cod){
    ...
    var curRec = CUR.players.find(function(p){ return String(p.cod)===String(cod); });
    ...
    $('#playerMeta').innerHTML =
      '<span class="'+cls+'">'+escapeHtml(roleKnown?role:(role||'—'))+'</span>'+
    ...
    if(PREV && prevRec){ var badges = buildRankingBadges(prevRec, idx); badges.forEach(function(x){ hdr.push(x); }); }
  ...
  // ==== Hotkeys & search wiring ====
  function wireSearch(){ var input=$('#searchInput'); if(!input) return; input.addEventListener('input', function(e){ var v=e.target.value; if(v && v.length>0) showResults(); else hideResults(); clearTimeout(wireSearch._t); wireSearch._t = setTimeout(function(){ search(v); }, 150); }); }
  document.addEventListener('keydown', function(e){ if((e.ctrlKey||e.metaKey) && (e.key==='q'||e.key==='Q')){ var el=$('#searchInput'); if(el){ e.preventDefault(); el.focus(); if(el.select) el.select(); showResults(); } } });
  ...
  function init(){
    ...
    discoverSeasons().then(function(list){
      SEASONS = list || [];
      if(!SEASONS.length){ var msg='Nessun file stagione trovato in: '+DATA_DIRS.join(' · ')+' (es. 2024_2025.json o 2024.json).'; console.warn(msg); alert(msg); return; }
      CUR  = SEASONS[SEASONS.length-1];
      PREV = SEASONS.length>=2 ? SEASONS[SEASONS.length-2] : null;
      $('#lblCurrent').textContent = 'Stagione ' + seasonStr(CUR.seasonKey);
      $('#lblPrev').textContent    = PREV ? ('Stagione ' + seasonStr(PREV.seasonKey)) : 'Stagione precedente';
      var dataInfo = $('#dataDirInfo'); if(dataInfo) dataInfo.textContent = 'Cartella dati rilevata: ' + (DATA_BASE || '(sconosciuta)');
      buildIndex();
