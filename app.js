...
(function(){
  ...
  // ==== Data: autodetect dir + 3 naming schemes ====
  ...
  function fileCandidatesFor(key){ var start=Number(String(key).split('_')[0]); var next=start+1; return [ key+'.json', start+'.json', (String(start)+'-'+String(next)+'.json') ]; }
  ...
  function tryFetch(path){ return fetch(path,{cache:'no-store'}).then(function(res){ if(!res.ok) return null; return res.json(); }).catch(function(err){ console.warn('fetch failed', path, err); return null; }); }
  ...
  // NEW: riga con label HTML ma valore formattato come intero (per conteggi)
  ...
  function renderCurrent(rec){
    ...
    // Interi per i conteggi richiesti
    if(isP){
